...
class TodoListManager:
    ...
    def add_task(self, name, priority="Normal", due_date="Sin fecha"):
        """
        Requisito: Añadir una nueva tarea.
        Cumple con el requisito de tener mínimo 4 atributos por tarea:
        1. Name, 2. Status, 3. Priority, 4. Due Date.
        """
        task = {
            "name": name,
            "status": "Pending",
    ...
    def list_tasks(self):
        ...
        print("\n--- Lista de Tareas ---")
        print(f"{'Nombre':<20} | {'Estado':<10} | {'Prioridad':<10} | {'Fecha'}")
        print("-" * 60)
        for task in self.tasks:
            print(f"{task['name']:<20} | {task['status']:<10} | {task['priority']:<10} | {task['due_date']}")
        return self.tasks
...
# Esto permite cumplir con la descripción de "command-line application" 
if __name__ == "__main__":
    ...
    while True:
        ...
        if choice == '1':
            ...
            date = input("Fecha límite (DD-MM-AAAA): ")
            manager.add_task(name, prio, date)
        elif choice == '2':
            manager.list_tasks()
        elif choice == '3':
